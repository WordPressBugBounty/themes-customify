...
(function ($, api) {
  ...
  wp.customize.selectiveRefresh.Partial.prototype.showControl = function () {
    ...
    if (settingId && parentWin && parentWin.wp && parentWin.wp.customize && typeof parentWin.customifyBuilderOpenSection === "function") {
      var matchedControl;
      parentWin.wp.customize.control.each(function (control) {
        if (matchedControl) return;
        var settings = control.settings || {};
        for (var key in settings) {
          if (Object.prototype.hasOwnProperty.call(settings, key) && settings[key] && settings[key].id === settingId) {
      ...
      if (matchedControl) {
        var sectionId = typeof matchedControl.section === "function" ? matchedControl.section() : matchedControl.section;
  ...
  function footerColPlacementRules(rowSelector, count, layout) {
    var spans = layout === '2-3' ? [3, 3, 2, 2, 2] : layout === '3-2' ? [2, 2, 2, 2, 2] : layout === '2-2-1' ? [3, 3, 3, 3, 3] : [];
    var css = '';
  ...
  function applyFooterColLayout(rowSelector, valueStr) {
    ...
    Object.keys(breakpoints).forEach(function (device) {
      ...
      var isIntentional = frLen === 1 || frLen === count || frLen > 1 && count > 0 && frLen < count && count % frLen === 0;
      ...
      var mixedRows = count === 5 && ['2-3', '3-2', '2-2-1'].indexOf(d.layout) !== -1 ? d.layout : '';
      var cols = mixedRows ? 'repeat(6, minmax(0, 1fr))' : fr.map(function (v) {
  ...
  var __footerRows = window.Customify_Preview_Config && window.Customify_Preview_Config.footer_rows || ['main', 'bottom'];
