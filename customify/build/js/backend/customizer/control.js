...
(function ($, wpcustomize) {
  ...
  var customifyField = {
          ...
          if (support_devices) {
            ...
            _.each(control.allDevices, function (device) {
              ...
              value[device] = {
                ...
                svg: $('input[data-name="' + _name + '-svg"]', $field).val() || ""
              };
            });
          } else {
            value = {
              ...
              svg: $('input[data-name="' + name + '-svg"]', $field).val() || ""
            };
          }
          break;
        case "radio":
    ...
    addDeviceSwitchers: function ($el) {
      ...
      $(".customify-field-heading", $el).not(".customify-devices-added").append(clone).addClass("customify-devices-added");
    },
 ...
 var c; 
    ...
    initColor: function ($el) {
      $(".customify-input-color", $el).each(function () {
        ...
        var current_val = linkedToken && linkedToken.preview ? linkedToken.preview : hidden.val();
        // data-alpha="true"
  ...
  var customifyPaletteLink = {
    config: function () {
      var args = typeof Customify_Control_Args !== "undefined" ? Customify_Control_Args : null;
      var cfg = args && args.palette_link ? args.palette_link : null;
      return cfg && cfg.enabled && cfg.tokens && cfg.tokens.length ? cfg : null;
    },
    ...
    nameOf: function (value) {
      var match = /^var\(\s*(--customify-[a-z0-9-]+)/i.exec(customifyPaletteLink.decode(value).trim());
    ...
    write: function (colorInput, value, previewHex) {
      ...
      if (previewHex) {
        ...
        $(".customify--color-panel", colorInput).wpColorPicker("color", previewHex);
    ...
    build: function (colorInput) {
      ...
      var valueInput = $('<input type="text" class="customify-palette-link__value-input" spellcheck="false" autocomplete="off" />');
      ...
      var copy = $('<button type="button" class="customify-palette-link__copy dashicons dashicons-admin-page"></button>').attr("title", cfg.l10n.copy);
      ...
      $.each(cfg.tokens, function (i, token) {
        var swatch = $('<button type="button" class="customify-palette-link__swatch"></button>').css("background-color", token.preview).attr("data-value", token.value).attr("data-label", token.label).attr("title", token.label);
        swatch.on("click", function (e) {
          e.preventDefault();
          e.stopPropagation();
          customifyPaletteLink.write(colorInput, token.value, token.preview);
        });
        row.append(swatch);
      });
      root.append(row);
    ...
    sync: function (colorInput) {
      ...
      var value = customifyPaletteLink.decode(customifyPaletteLink.hidden(colorInput).val() || "").trim();
      ...
      if (chip.length) {
        if (token && token.preview) {
          chip[0].style.setProperty("background-color", token.preview, "important");
      ...
      $(".customify-palette-link__swatch", row).each(function () {
        var swatch = $(this);
        swatch.toggleClass("is-active", !!activeName && customifyPaletteLink.nameOf(swatch.attr("data-value")) === activeName);
      });
      ...
      if (token) {
        ...
        var unlink = $('<button type="button" class="customify-palette-link__unlink"></button>').text(cfg.l10n.unlink);
        unlink.on("click", function (e) {
          e.preventDefault();
          e.stopPropagation();
          // Hand the field back to Iris: keep the colour identical,
          // drop the link.
          customifyPaletteLink.write(colorInput, token.preview || "", token.preview);
        });
        status.addClass("is-linked").append($("<span></span>").text(cfg.l10n.linked + " ")).append(dot).append($("<strong></strong>").text(token.label)).append(unlink);
      } else {
  ...
  var IconPicker = {
    ...
    render: function (list_icons) {
      ...
      if (!$typeSelect.find('option[value="custom-svg"]').length) {
        var svgLabel = typeof Customify_Control_Args !== "undefined" && Customify_Control_Args.custom_svg_label ? Customify_Control_Args.custom_svg_label : "Custom SVG";
        $typeSelect.append(' <option value="custom-svg">' + svgLabel + "</option>");
      }
    },
    // The preset map, or {} when PHP shipped nothing (Pro filtered the
    // library empty, or an old bundle is running against new PHP).
    svgPresets: function () {
      return typeof Customify_Control_Args !== "undefined" && Customify_Control_Args.svg_icons ? Customify_Control_Args.svg_icons : {};
    },
    ...
    renderSuggested: function ($picker) {
      ...
      var keys = raw ? raw.split(",").map(function (k) {
        return $.trim(k);
      }) : [];
      var library = that.svgPresets();
      var selectedType = $picker ? $(".customify--input-icon-type", $picker).val() || "" : "";
      var selectedIcon = $picker ? $(".customify--input-icon-name", $picker).val() || "" : "";
      var rendered = 0;
      _.each(keys, function (key) {
        ...
        $list.append('<li class="customify--icon-suggested-item' + (isActive ? " is-selected" : "") + '" data-type="svg" data-icon="' + _.escape(key) + '" title="' + _.escape(library[key].label || key) + '"><span class="icon-wrapper">' + library[key].svg + "</span></li>");
      });
    ...
    addSvgPresets: function () {
      ...
      var typeLabel = typeof Customify_Control_Args !== "undefined" && Customify_Control_Args.svg_icons_label ? Customify_Control_Args.svg_icons_label : "SVG Icons";
      var $typeSelect = $("#customify--sidebar-icon-type");
      ...
      var html = '<ul class="customify--list-icons customify--list-svg-icons icon-svg" data-type="svg">';
      _.each(presets, function (item, key) {
        ...
        html += '<li title="' + _.escape(label) + '" data-type="svg" data-icon="' + _.escape(key) + '" data-label="' + _.escape(label.toLowerCase()) + '"><span class="icon-wrapper">' + markup + "</span></li>";
      });
      html += "</ul>";
    ...
    filterIcons: function (term) {
      ...
      $browser.find('li[data-icon*="' + safe + '"], li[data-label*="' + safe.toLowerCase() + '"]').show();
    },
    show: function () {
    ...
    picker: function () {
      ...
      $document.on("click", "#customify--icon-browser li, .customify--icon-suggested-item", function (e) {
        e.preventDefault();
      ...
      $document.on("click", ".customify--icon-custom-svg-apply", function (e) {
        e.preventDefault();
        ...
        var svgLabelApply = typeof Customify_Control_Args !== "undefined" && Customify_Control_Args.custom_svg_label ? Customify_Control_Args.custom_svg_label : "Custom SVG";
        $(".customify--input-icon-name", that.pickingEl).val(svgLabelApply).trigger("change");
      ...
      $document.on("click", ".customify--icon-custom-svg-clear", function (e) {
        e.preventDefault();
    ...
    selectIcon: function ($li) {
      ...
      $(".customify--input-icon-name", this.pickingEl).val(icon).trigger("change");
      $(".customify--icon-preview-icon", this.pickingEl).html(preview);
      $(".customify--icon-suggested-item").removeClass("is-selected");
      $('.customify--icon-suggested-item[data-icon="' + icon.replace(/["\\]/g, "") + '"]').addClass("is-selected");
      this.close();
    },
    ...
    sanitizeSvg: function (raw) {
      ...
      raw = raw.replace(/^<svg\b[^>]*>/i, function (tag) {
        return tag.replace(/\s(width|height)\s*=\s*("[^"]*"|'[^']*'|[^\s>]+)/gi, "");
      });
      return raw;
    },
